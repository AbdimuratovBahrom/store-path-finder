...
# === Импорт init_db для инициализации БД ===
from init_db import init_database  # <-- НОВОЕ
...
@app.route("/")
def index():
    # Инициализация БД при старте (если нет или маленькая)
    if not os.path.exists("shops.db") or os.path.getsize("shops.db") < 10000:
        print("Инициализация БД при запуске...")
        init_database()
...
def get_stores(block, row):
    conn = get_db_connection()
    if block in ["Гипермаркет", "Специфические объекты"]:
        shops = conn.execute("SELECT DISTINCT shop FROM shops WHERE block=?", (block,)).fetchall()
        conn.close()
        if block == "Специфические объекты":
            items = [s["shop"] for s in shops if not is_excluded_for_specific(s["shop"])]
            return jsonify({"items": sorted(items, key=str.lower)})
        return jsonify({"items": sorted({s["shop"] for s in shops}, key=numeric_key_for_shop)})
...
def get_path(block, row, shop):
    ...
    paths = [r["path"].strip() for r in res]
    paths = [p if p.endswith(shop) else f"{p} > {shop}" for p in paths]
    return jsonify({"path": " | ".join(paths)})
